      ...
      `}>
              ...
              {AVAILABLE_MODELS.map(model => (
                ...
                >
                  <div className="flex items-start justify-between mb-1">
                    <span className={`font-medium text-sm ${selectedModel === model.id ? 'text-blue-400' : 'text-zinc-200'}`}>
                      {model.name}
                    </span>
                    {selectedModel === model.id && (
        ...
        <div className="p-4 border-t border-zinc-800 bg-zinc-900/50">
          <div className="flex items-center justify-between text-xs text-zinc-500">
            <a href="https://webllm.mlc.ai" target="_blank" rel="noopener noreferrer" className="hover:text-blue-400 transition-colors flex items-center gap-1">
              Powered by WebLLM
            </a>
            <span>v1.0.0</span>
          </div>
